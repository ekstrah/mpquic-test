...
7. **Send failures were indistinguishable from successes.** `picoquic_queue_datagram_frame`/`picoquic_add_to_stream`'s return values were discarded, so a dropped chunk under congestion looked identical to a delivered one from the app's own output. Fixed with generated-vs-queued counters on both channels, reported at close (`duration elapsed, closing (video: X/Y chunks queued)`).

### 5.3 First real measurement, and what it shows

A 30-second `cubic` run, taken *after* bug #5 was fixed (the only run so far with congestion control actually active):
...
**Not yet done:** a CC-algorithm sweep under this traffic model (bbr, newreno, bbr1, fast, dcubic, prague, c4) to see whether the path-imbalance pattern holds across schedulers, and whether it interacts with the still-open `PROTOCOL_VIOLATION` truncation bug from §4.3 (not yet seen under this traffic model — datagrams don't retransmit and the control stream's data volume is far smaller than the bulk-transfer tests that surfaced it, so the conditions that trigger it may simply not arise here as often).
...
6. **From §5.3: run a CC-algorithm sweep under the real traffic model** (the single `cubic` run so far is the only one with congestion control actually active — see §5.2 bug #5) to check whether the observed Link-C-dominance/Link-B-starvation pattern holds across schedulers, and whether it correlates with §4.3's still-open truncation bug.
