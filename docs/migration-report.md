...
1. **Root cause confirmed (§4.3): a `dcubic` truncation's client qlog shows `PROTOCOL_VIOLATION` triggered by a path-scoped `RETIRE_CONNECTION_ID` frame retiring the connection ID currently in active use on path 1, with retirements issued out of sequence order.** This is a picoquic multipath connection-ID-rotation race, not a harness/shaping artifact. Remaining work here: reproduce a couple more times to build confidence before considering an upstream bug report, and check whether it's specific to path 1 (Link B, the highest-RTT link) or coincidental.
