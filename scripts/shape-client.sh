...
sudo tc qdisc add dev "$CLIENT_IFACE_A" root handle 1: tbf rate ${LINK_A_RATE_UP_MBIT}mbit burst 32kbit latency 400ms
...
sudo tc qdisc add dev "$CLIENT_IFACE_B" root handle 1: tbf rate ${LINK_B_RATE_MBIT}mbit burst 32kbit latency 400ms
...
sudo tc qdisc add dev "$CLIENT_IFACE_C" root handle 1: tbf rate ${LINK_C_RATE_MBIT}mbit burst 32kbit latency 400ms
...
echo "client-side shaping applied:"
